...
- **`async_boot_providers()`** runs on server startup, calling `async_boot()` on every provider that overrides it, in registration order. It is idempotent *within* a lifespan — a repeated startup without an intervening shutdown is a no-op.
...
Startup and teardown are symmetric: shutdown clears the async-boot flag, so a second lifespan in the same process boots again rather than reusing resources the first teardown already released. This matters for tests that open two `TestClient` blocks and for deployment patterns that restart the ASGI app in-process — a provider that acquires in `async_boot()` and releases in `shutdown()` gets a fresh resource each time.
