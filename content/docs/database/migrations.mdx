...
<Callout type="info">
`json()` maps to the dialect's *parsed* JSON type — `JSONB` on PostgreSQL, native
`JSON` on MySQL — so the stored value is decoded and queryable the same way
everywhere: text searches see real characters (not `\uXXXX` escapes), and
PostgreSQL gains GIN indexing and containment (`@>`) queries. The trade-off is
jsonb semantics: object key order is not preserved and duplicate keys collapse.
If you need PostgreSQL's raw text-preserving `json` type, create the column with
`self.execute()` in a migration.
</Callout>
...
| `.case_sensitive()` | Compare this text column exactly — see below |
...
MySQL's default collation (`utf8mb4_0900_ai_ci`) compares text case- *and*
accent-insensitively, so `User:ABC`, `user:abc` and `user:abç` are the same
value. For a display name that is usually what you want; for a key it is a
bug — one principal reads another's cached entry, separate rate-limit buckets
merge into one, and a lookup matches a row it was never meant to.

`.case_sensitive()` asks for exact comparison and lets each backend decide
what that requires:

```python
with schema.create("api_tokens") as table:
    table.string("token").primary().case_sensitive()
```

MySQL gets `CHARACTER SET utf8mb4 COLLATE utf8mb4_bin`. PostgreSQL and SQLite
already compare byte-for-byte, so nothing is emitted for them — the modifier
states the requirement rather than a collation name, so the same migration is
correct on every backend. It applies on both the create and ALTER paths, and
raises on a non-text column rather than passing silently on the two backends
...
- **Column comments are silently unavailable on SQLite.** SQLite has no column-comment syntax and stores nothing for one, so `table.comment(...)` and `schema.comment(...)` run without error and keep nothing. MySQL and PostgreSQL both persist them: MySQL has no standalone `COMMENT ON`, so the framework reads the column's current definition and restates it — type, nullability, default, character set and collation, `AUTO_INCREMENT`, generation expression and spatial SRID all survive.
